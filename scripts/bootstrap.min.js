 ...
 * Implementation
 ******************/
        var audioContextCheck = typeof AudioContext !== 'undefined' ? AudioContext : typeof webkitAudioContext !== 'undefined' ? webkitAudioContext : undefined;
        // Shared variables
        var DOMReadyTime, loadingScreen, hipsterWhale, wrapper, loadingDone, brandingDone, started;
        // Create the wrapper that will contain the preroll, loader and game
        ...
        // Phase 1: Initializiation
        // ------------------------
        onDOMReady(function() {
            // for testing add build number JAB
            ...
            //analytics.track('game_onboarding', 'loading', 'dom_ready');
            DOMReadyTime = performance.now();
            window.bodyRect = document.body.getBoundingClientRect();
            console.log("BodyRect dimensions onDOMReady:", bodyRect.width, bodyRect.height);
            // Setup wrapper
            document.body.appendChild(wrapper);
            resize();
            window.addEventListener('resize', resize);
            // Load the AssetLoader in order to start loading the preroll
            loadScript('scripts/asset-loader.min.js', loadPreroll);
        });
        // Run window.onload after all content (images, styles) is fully loaded
        window.onload = function() {
            console.log("All resources finished loading");
            // Set bodyRect dimensions, as it may depend on images or styles being loaded
            window.bodyRect = document.body.getBoundingClientRect();
            console.log("BodyRect dimensions .onload:", bodyRect.width, bodyRect.height);
        }
        ;
        // Phase 2: Preroll
        // ------------------------
        function showHipsterSplash() {
            //analytics.track('game_onboarding', 'loading', 'game_developer_branding');
            hipsterWhale.show();
        ...
        function loadPreroll() {
            console.log("2. loadPreroll");
            // First up, load what's required for our preroll
            AssetLoader.add.script('scripts/loading-flow.min.js');
            AssetLoader.add.image('sprites/poki-icon.png');
            AssetLoader.add.image('sprites/poki-logo.png');
            AssetLoader.load(function() {
                //analytics.track('game_onboarding', 'loading', 'poki_branding');
                // Requirements loaded, create and start the preroll
                var preroll = new PokiPreroll();
                preroll.wrapper = wrapper;
                preroll.logoSrc = AssetLoader.getAssetById('sprites/poki-logo.png').src;
                preroll.createElements();
                if (!skipPreroll) {
                    // Show HipsterWhale after preroll has faded out
                    preroll.play(showHipsterSplash);
                }
                // While the preroll runs, load the required assets for the hipster whale screen
                ...
                AssetLoader.load(function() {
                    // Done loading! Tell the preroll it can close, and start loading the game
                    preroll.allowClose();
                    hipsterWhaleBranding();
                });
            });
            // Start loading three.min.js already as it is relatively large
            // even though we don't need it until the loader
            AssetLoader.loader.script('scripts/three.min.js');
        }
        ;function hipsterWhaleBranding() {
            console.log("3. HW Branding");
            hipsterWhale = new HipsterWhale();
            hipsterWhale.wrapper = wrapper;
            hipsterWhale.logoSrc = AssetLoader.getAssetById('sprites/hipster-whale.png');
            hipsterWhale.hipsters = [AssetLoader.getAssetById('sprites/hipster-1.png'), AssetLoader.getAssetById('sprites/hipster-2.png'), AssetLoader.getAssetById('sprites/hipster-3.png')];
            hipsterWhale.whales = [AssetLoader.getAssetById('sprites/whale-1.png'), AssetLoader.getAssetById('sprites/whale-2.png'), AssetLoader.getAssetById('sprites/whale-3.png')];
            // Load additional assets required for loading screen
            ...
            // Still add the load here to make sure it's done before we move on
            AssetLoader.load(function() {
        ...
        // ------------------------
        function loadGame() {
            console.log("3. loadGame");
            //analytics.track('game_onboarding', 'loading', 'game_loader_start');
            ...
            AssetLoader.progressListener = loadingScreen.updateProgress.bind(loadingScreen);
            AssetLoader.maxConcurrency = 5;
            // Scripts
            AssetLoader.add.script('scripts/game.min.js');
            // Assets
            AssetLoader.add.spriteSheet('sprites/CrossyUI.png', 'sprites/CrossyUI.json');
            ...
            if (!true || typeof audioContextCheck === 'undefined' || typeof audioContextCheck.prototype === 'undefined' || typeof audioContextCheck.prototype.decodeAudioData === 'undefined') {
                ...
                // Our browser is next-level, serve the audio file archive instead to reduce requests
                AssetLoader.setupAudioLoader();
                AssetLoader.add.archive('audio.zip', function(zip) {
                    Object.keys(zip.files).forEach(function(key) {
                        var file = zip.files[key];
                        // Make sure we're only using the mp4 files
                        if (file.name.split('.').pop() !== 'mp4')
                            return;
                        file.async('base64').then(function(content) {
                            AssetLoader.audioLoader.load('data:audio/mp4;base64,' + content, function(buffer) {
                                AssetLoader.loadedAssets[file.name] = buffer;
                            });
                        });
                    });
                });
            ...
            // Trigger load, and start game when done
            AssetLoader.load(function() {
                loadingScreen.forceSkip = true;
                // Tell the loading screen we're done and we can skip
                loadingDone = true;
                attemptStart();
            });
        }
        ...
        function startGame() {
            ...
            //analytics.track('game_onboarding', 'loading', 'game_loader_finished');
 ...
 */
        function resize() {
            ...
            //analytics.track('technical_performance', 'resize_body', bodyRect.width + 'x' + bodyRect.height);
        }
        ;/**
