...
*/
    ...
    (function(module, exports, __webpack_require__) {
        ...
        window.HipsterWhale = __webpack_require__(5);
        window.LoadingScreen = __webpack_require__(6);
        window.PokiPreroll = __webpack_require__(7);
...
*/
        {
            ...
            var FadeHelper = __webpack_require__(1);
            // Config
            ...
            HipsterWhale.prototype.play = function(callback) {
                ...
                this.resize();
                for (var b = 0; b < 20; b++) {
                    var bx = getRandomArbitrary(0, this.canvas.width);
                    var by = getRandomArbitrary(0, this.canvas.height);
                ...
                window.setTimeout(this.whaleMovement.bind(this), 10);
                //was 250
                this.animate();
                // JAB mute
                //
...
*/
                ...
                //console.log(whaleX+","+whaleY+" -> "+dx+","+dy);
                var coord = {
                    y: whaleY
                };
                if (whaleTween)
                    whaleTween.stop();
                whaleTween = new TWEEN.Tween(coord).to({
                    y: dy
            ...
            HipsterWhale.prototype.drawBubbles = function() {
                for (var b = 0; b < this.bubbles.length; b++) {
                    ...
                    bubble.y -= 10 * this.deltaTime / this.canvas.height;
                    if (bubble.y < 0)
                        bubble.y = this.canvas.height;
                    this.context.fillRect(bubble.x, bubble.y, bubble.size, bubble.size);
                }
            }
            ;
...
*/
        {
            var LoadingScreen = function LoadingScreen() {
                ...
                this.hipsterSrc = [AssetLoader.getAssetById('sprites/hipster-1.png').src, AssetLoader.getAssetById('sprites/hipster-2.png').src, AssetLoader.getAssetById('sprites/hipster-3.png').src];
                this.whalesSrc = [AssetLoader.getAssetById('sprites/whale-1.png').src, AssetLoader.getAssetById('sprites/whale-2.png').src, AssetLoader.getAssetById('sprites/whale-3.png').src];
                window.addEventListener('resize', this.resize.bind(this));
                //this.animate();
            };
            var FadeHelper = __webpack_require__(1);
            ;LoadingScreen.prototype.updateProgress = function(progress) {
                console.log("LoadingScreen.updateProgress");
                //JAB debug
                //
                //console.log("Loading screen updateProgress called. Current progress:", progress);
                this.progress = progress;
                var shownProgress = 1;
                if (this.progressZeroPoint < 1) {
                    shownProgress = (this.progress - this.progressZeroPoint) / (1 - this.progressZeroPoint);
                }
                if (this.progressText && this.shown) {
                    //JAB debug
                    //
                    //console.log("Updating progress text:", parseInt((shownProgress * 100), 10) + '%');
                    this.progressText.innerText = parseInt(shownProgress * 100, 10) + '%';
                }
            }
            ;
	...
	if(this.textTimer>=60){
		...
		this.textTimer=0;
            ...
            LoadingScreen.prototype.show = function(skippedPreroll) {
                console.log("LoadingScreen.show");
                //JAB debug
                //
                ...
                if (this.progress >= 1 || this.forceSkip) {
                    ...
                    if (this.destroyed)
                        return;
                    this.destroyed = true;
                    // We're already done loading, move on!
                    this.wrapper.removeChild(this.container);
                    return;
                }
                this.startTime = performance.now();
                //console.log("Loading screen start time set:", this.startTime);
                // Fade in and reset progress zero point
                this.progressZeroPoint = this.progress;
                this.updateProgress(this.progress);
                FadeHelper.fadeTo(this.fadeRectangle, 0);
            }
            ;
            LoadingScreen.prototype.createGameLogo = function() {
                //JAB don't do HW splash
                //
                if (false) {
                    this.hipsterText = new Image();
                    this.hipsterText.src = this.hipsterSrc[0];
                    this.whaleText = new Image();
                    this.whaleText.src = this.whalesSrc[0];
                    var maxWidth = bodyRect.width * 0.4;
                    var maxHeight = bodyRect.height * 0.2;
                    if (maxWidth / this.textAspect > maxHeight) {
                        this.textAspect = this.hipsterText.width / this.hipsterText.height;
                        this.hipsterText.height = maxHeight;
                        this.hipsterText.width = maxHeight * this.textAspect;
                        this.textAspect = this.whaleText.width / this.whaleText.height;
                        this.whaleText.height = maxHeight;
                        this.whaleText.width = maxHeight * this.textAspect;
                    } else {
                        this.textAspect = this.hipsterText.width / this.hipsterText.height;
                        this.hipsterText.width = maxWidth;
                        this.hipsterText.height = maxWidth / this.textAspect;
                        this.textAspect = this.whaleText.width / this.whaleText.height;
                        this.whaleText.width = maxWidth;
                        this.whaleText.height = maxWidth / this.textAspect;
                    }
                    var setStyle = function setStyle(txt) {
                        txt.style.position = 'absolute';
                        txt.style.top = '15%';
                        txt.style.margin = '0 auto';
                        txt.style.left = '0';
                        txt.style.right = '0';
                        txt.style.maxHeight = '40%';
                        txt.style.maxWidth = '60%';
                    };
                    setStyle(this.hipsterText);
                    setStyle(this.whaleText);
                    this.whaleText.style.top = '35%';
                    this.container.appendChild(this.hipsterText);
                    this.container.appendChild(this.whaleText);
                }
            }
            ;
            LoadingScreen.prototype.createPokiLogo = function() {
                //JAB force not to create poki logo
                //
                if (false) {
                    var anchor = document.createElement('a');
                    anchor.href = 'http://poki.com/';
                    anchor.target = '_blank';
                    anchor.style.position = 'absolute';
                    anchor.style.display = 'block';
                    anchor.style.bottom = '10%';
                    anchor.style.margin = '0 auto';
                    anchor.style.left = '0';
                    anchor.style.right = '0';
                    anchor.style.height = '10%';
                    anchor.style.maxWidth = '40%';
                    anchor.style.backgroundImage = 'url(' + this.pokiLogoSrc + ')';
            ...
            ;
            LoadingScreen.prototype.createProgressIndicator = function() {
                var progressText = document.createElement('div');
                progressText.innerText = '0%';
                progressText.style.position = 'absolute';
                progressText.style.height = '30px';
                progressText.style.top = '60%';
                progressText.style.margin = '0 auto';
                progressText.style.left = '0';
                progressText.style.right = '0';
                progressText.style.fontFamily = 'EditUndoBrk';
            ...
            LoadingScreen.prototype.resize = function() {
                ...
                this.progressText.style.fontSize = parseInt(containerBounds.height / 16) + 'px';
            }
            ;
            LoadingScreen.prototype.createFadeRectangle = function() {
                var fadeRectangle = document.createElement('div');
                fadeRectangle.style.position = 'absolute';
                fadeRectangle.style.left = '0';
                fadeRectangle.style.top = '0';
                fadeRectangle.style.right = '0';
                fadeRectangle.style.bottom = '0';
                fadeRectangle.style.zIndex = '2';
                fadeRectangle.style.backgroundColor = this.bgColor;
                fadeRectangle.style.opacity = '1';
                fadeRectangle.style.pointerEvents = 'none';
                this.container.appendChild(fadeRectangle);
                this.fadeRectangle = fadeRectangle;
            }
            ;
            LoadingScreen.prototype.destroy = function() {
                ...
                if (!this.shown) {
                    //JAB debug
                    //
                    console.log("Loading screen not shown. Removing container.");
                    this.wrapper.removeChild(this.container);
                    return;
                }
                FadeHelper.fadeTo(this.fadeRectangle, 1, function() {
                    FadeHelper.fadeTo(this.container, 0, function() {
                        //JAB debug
                        //
                        //console.log("Fading out loading screen container.");
                        console.log("Inside fadeTo callback. `this` is:", this);
                        this.wrapper.removeChild(this.container);
...
*/
        {
            ...
            PokiPreroll.prototype.createElements = function() {
                ...
                ['-100%', '0%', '100%'].forEach(function(left) {
            ...
            PokiPreroll.prototype.play = function(callback) {
                ...
                .bind(this), 50);
                //was 1500 with poki, changed to 50 to skip poki logo
                //JAB
                //this.end();
            }
            ;
            ...
            PokiPreroll.prototype.resize = function() {
                ...
                // Find the closest amount of icons we can fit, pixels get truncated so
                // we want to avoid broken edges
                var leastDifference = Infinity;
                var leastAmount = void 0;
                [7, 8, 9, 10, 11].forEach(function(amount) {
